  ...
  return (
    <div className="pt-4 px-4 pb-10 max-w-md mx-auto">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold text-orange-500 text-center flex-1">
          {item.title}
        </h1>
      ...
      <div className="flex items-center gap-3 p-3 rounded-xl shadow bg-white mb-4">
        ...
        <div className="flex-1">
          ...
          {user && item.userId === user.uid ? (
            ...
            <p className="text-xs text-gray-500">{t("messageSeller")}</p>
          )}
        </div>
        {user && item.userId !== user.uid && (
          <button
          ...
          >
            {t("messageSeller", { userName: item.userName })}
          </button>
        )}
        {!user && (
          <p className="text-sm text-red-500">{t("loginToMessageSeller")}</p>
        )}
      </div>
          ...
          <div className="bg-white rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <form onSubmit={handleFlagSubmit}>
                ...
                <div className="mb-6">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {t("reportItemCommentLabel")}
                  </label>
                  <textarea
                    value={flagComment}
                    onChange={(e) => setFlagComment(e.target.value)}
                    placeholder={t("reportItemPlaceholder")}
                ...
                <div className="flex space-x-3">
                  ...
                  >
                    {isSubmittingFlag
                      ? t("reportItemSubmittingButton")
                      : t("reportItemSubmitButton")}
                  </button>
                </div>
              </form>
            </div>
