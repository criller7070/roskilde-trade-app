...
const ChatList = () => {
  ...
  const { t } = useTranslation("chatList"); // Specify the 'chatList' namespace
  ...
  const deleteChat = async (chatId, itemName) => {
    try {
      ...
      if (import.meta.env.DEV) {
        console.error(t("errorDeletingChat"), error.code);
      }
      showError(t("errorDeletingChat"));
    }
  };

  const confirmDeleteChat = (chatId, itemName) => {
    showConfirm(
      t("confirmDeleteChat", { itemName }),
  ...
  useEffect(() => {
    const checkDeletedItems = async () => {
      ...
      for (const chat of chats) {
        if (chat.itemId && !chat.itemName?.includes(t("deletedTag"))) {
          try {
          ...
          statusChecks[chat.id] = {
            isDeleted: chat.itemName?.includes(t("deletedTag")) || false,
            itemName: chat.itemName,
          };
        }
      }
  ...
  if (loading) {
    return (
      <div className="pt-20 px-4 text-center">
        <p className="text-gray-600">{t("loadingChats")}</p>
      </div>
    );
  }
                ...
                <Link to={`/chat/${chat.id}`} className="block">
                  <div className="flex items-center space-x-4 pr-12">
                    {chat.itemImage && (
                      <div
                        ...
                        className={`flex-shrink-0 ${
                          !isDeleted && chat.itemId
                            ? "cursor-pointer hover:opacity-80 transition-opacity duration-200"
                            : ""
                        }`}
                        title={!isDeleted && chat.itemId ? t("clickToViewItem") : ""}
                      >
                        <LoadingPlaceholder
                          src={chat.itemImage}
                          alt={chat.itemName}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        {t("withUser", { userName: chat.otherUserName })}
                      </p>
                      <p
